![[src/img/Pasted image 20210729135003.png]]


# 知识点
1. `random()`可以直接随机数组。比如`random([apple,pear])`.直接random不传参可以返回一个0~1的浮点数。
2. noloop写在setup里，然后用key来设置按空格重绘draw()
3. `arc(x,y,w,h,start,end)` 旋转的起点是水平右边，旋转方向是**顺时针**。`2PI`会回到起点。如果写`arc(x,y,wh,3*PI/2,PI/2)`会画出一个右边的圆，但是调换起点终点的话会画出左半圆。

![[src/img/IMG_7067-压缩.jpg]]
## 代码
